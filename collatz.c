...
unsigned int collatz_provider(unsigned int num)
{
...
unsigned int collatz_lru(unsigned int num)
{
    ...
    for (unsigned int i = 0; i < array_size; ++i)
    {
        ...
        if (num == index_array[i])
        {
            ...
            for (unsigned int j = array_size; j > 0; --j)
            {
    ...
    unsigned int step_count = collatz_provider(num);

    if (array_size == Max_array_size)
    {

        unsigned int last_elem_pos = 0;
...
unsigned int collatz_rr(unsigned int num)
{
    ...
    if (array_size == Max_array_size)
    {
        ...
        unsigned int rand_pos = rand() % ((array_size - 1) + 1); //'array_size - 1' is max to make sure we have space
...
int main(unsigned int argc, char *argv[])
{
    ...
    for (unsigned int i = 0; i < n; ++i)
    {
        ...
        unsigned int step_count = collatz(rand_num);
    ...
    printf("\n\nDEBUG : array_size = %u", array_size);

    printf("\nCache hits = %u, Cache misses = %u, N = %u", cache_hits, cache_misses, n);
